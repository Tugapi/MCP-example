...
class ChatSession:
    def __init__(self, llm_client: LLMClient, mcp_session: ClientSession) -> None:
    ...
    async def process_llm_response(self, llm_response: str) -> str:
        ...
        try:
            ...
            if "tool" in tool_call and "arguments" in tool_call:
                # 检查工具列表是否包含该工具
                tool_response = await self.mcp_session.list_tools()
...
async def main():
    async with AsyncExitStack() as exit_stack:
        server_params = StdioServerParameters(
            command="python", args=["fastmcp_stdio_server.py"], env=None
        )
        # 建立stdio通信
